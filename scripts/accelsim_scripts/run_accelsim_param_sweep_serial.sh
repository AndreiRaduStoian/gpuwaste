...
TRACE_ROOT="/workspace/gpuwaste/accelsim_traces/synthetic_reduced/12.9/"
BENCHMARK="synthetic_reduced"
...
wait_for_run() {
    ...
    while true; do
        ...
        # job_status.py does not print the Passed/Running summary.
        # A synthetic_reduced run has 5 jobs. When all five are COMPLETE_NO_OTHER_INFO,
        # the run is done.
        ...
        if [[ "${complete_count}" -ge 5 && "${running_count}" -eq 0 && "${waiting_count}" -eq 0 ]]; then
