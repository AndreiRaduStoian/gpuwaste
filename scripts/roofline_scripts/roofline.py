...
def plot_rooflines(
    ...
    for machine in machines:
        y = roofline_performance(machine, x) / 1e9
    ...
    if kernels:
        for kernel in kernels:
            if kernel.measured_ops_per_sec is not None:
                y = kernel.measured_ops_per_sec / 1e9
            else:
...
def make_kernel_point(
    name: str,
    total_ops: float,
    total_bytes: float,
    runtime_ms: Optional[float] = None,
) -> KernelPoint:
    operational_intensity = total_ops / total_bytes if total_bytes > 0 else float("inf")

    measured_ops_per_sec = None
    if runtime_ms is not None:
        measured_ops_per_sec = total_ops / (runtime_ms / 1000.0)
